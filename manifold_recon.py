...
def forward_model(X_train,Y_train,model_str):
    with tf.device('/gpu:0'):
...
#        config.gpu_options.all_growth = True
       # config = tf.ConfigProto(log_device_placement = True)
        with tf.Session() as sess:
            ...
            sess.close()
    return Y_opt 
#model_str = "model_t/model_maniflod_spiral.ckpt"
#Y_test = forward_model(X_train, Y_train,model_str)

#pl.ImagePlot(Y_test)
#dir = 'test_spiral'+time.strftime("%d-%m-%Y_%H-%M-%S")+'.npy'
